...
class SpikeProfitStrategy:
    """
    Spike止盈策略：
    - 入场：MA5>MA10, consecutive>=3
    - 首批建仓：40股（快速建仓）
    - 加仓：趋势延续时继续加仓
    - Spike止盈：当价格突然上涨>=spike_threshold时，卖出部分仓位
    - 止损：跌破入场价-1.5%
    """

    def __init__(self, first_position=40, spike_threshold=10,
                 stop_loss_pct=0.015, trailing_stop_pct=0.025):
        self.position_limit = 80
        self.first_position = first_position
        self.spike_threshold = spike_threshold  # 价格突刺阈值（点）
    ...
    def signal(self, state: TradingState, product: str, position: int) -> List[Order]:
        ...
        if position == 0:
            ...
            # Spike止盈：价格上涨超过阈值时卖出一半
            if price_change >= self.spike_threshold and position >= 20:
                sell_half = position // 2
                if sell_half > 0:
                    orders.append(Order(product, int(mid_price), -sell_half))
                    self.entry_price[product] = mid_price  # 更新入场价
                    return orders
...
def run_strategy(intarian_data, first_position, spike_threshold, stop_loss_pct, trailing_stop_pct):
    exchange = SimulatedExchange()
    exchange.reset()
    strategy = SpikeProfitStrategy(
        first_position, spike_threshold, stop_loss_pct, trailing_stop_pct
    )
...
def main():
    ...
    print("Spike止盈策略测试")
    print("=" * 80)
    print(f"{'First':<8} {'Spike':<8} {'StopLoss':<10} {'Trail':<10} {'AvgPnL':<12}")
    print("-" * 80)

    best_pnl = -float('inf')
    best_config = None

    for first_pos in [20, 40, 60]:
        for spike in [5, 8, 10, 12, 15]:
            for stop_loss in [0.010, 0.015]:
                for trailing in [0.020, 0.025]:
                    pnls = []
                    for folder_name, intarian_data in all_data:
                        pnl = run_strategy(
                            intarian_data, first_pos, spike, stop_loss, trailing
                        )
                        pnls.append(pnl)
                    ...
                    if avg_pnl > best_pnl:
                        best_pnl = avg_pnl
                        best_config = (first_pos, spike, stop_loss, trailing)
                        print(f"{first_pos:<8} {spike:<8} {stop_loss*100:<8.1f}% {trailing*100:<8.1f}% {avg_pnl:<12.2f} *")

    print("-" * 80)
    if best_config:
        print(f"\n最优配置: First={best_config[0]}, Spike={best_config[1]}, StopLoss={best_config[2]*100:.1f}%, Trail={best_config[3]*100:.1f}%")
