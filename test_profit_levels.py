...
class StrategyWithProfitTaking:
    def __init__(self, first_position=20, profit_take_pct=0.025, stop_loss_pct=0.015):
    ...
    def signal(self, state: TradingState, product: str, position: int) -> List[Order]:
        ...
        if position == 0:
        ...
        elif position > 0 and self.entry_price > 0:
            profit_pct = (mid_price - self.entry_price) / self.entry_price
            ...
            # 止损
            if mid_price < self.entry_price * (1 - self.stop_loss_pct):
                orders.append(Order(product, int(mid_price), -position))
                self.num_trades += 1
                self.entry_price = 0
                self.consecutive_uptrend = 0
...
def run_strategy(intarian_data, first_position, profit_take_pct, stop_loss_pct):
    ...
    exchange.reset()
    strategy = StrategyWithProfitTaking(first_position, profit_take_pct, stop_loss_pct)
    ...
    return exchange.get_final_value(intarian_data), strategy.num_trades
...
def main():
    ...
    print("测试止盈阈值对PnL的影响")
    print("=" * 80)
    print(f"{'FirstPos':<10} {'ProfitTake':<12} {'StopLoss':<10} {'AvgPnL':<12} {'Trades':<8}")
    print("-" * 80)
    ...
    for first_pos in [20, 40, 60]:
        for profit_take in [0.005, 0.010, 0.015, 0.020, 0.025, 0.030, 0.050]:
            for stop_loss in [0.010, 0.015, 0.020]:
                pnls = []
                total_trades = 0
                for folder_name, intarian_data in all_data:
                    pnl, trades = run_strategy(intarian_data, first_pos, profit_take, stop_loss)
                    pnls.append(pnl)
                    total_trades += trades
                ...
                if avg_pnl > best_pnl:
                    ...
                    print(f"{first_pos:<10} {profit_take*100:<10.1f}% {stop_loss*100:<10.1f}% {avg_pnl:<12.2f} {total_trades:<8} *")
